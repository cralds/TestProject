...
public class WaveView extends View
{
     ...
     * 水波平移速度
     */
    public static final float SPEED = 8.7f;

    private List<Point> mPointsList;
    private Paint mPaint,mPaint1;
    private Paint mTextPaint;
    private Path mWavePath,mWavePath1;
...
//            mWaveHeight = mViewWidth / 2.5f;
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        mWavePath1.moveTo(mPointsList.get(0).getX(), mPointsList.get(0).getY());
        for (; i < mPointsList.size() - 2; i = i + 2)
        {
            ...
            mWavePath1.quadTo(mPointsList.get(i + 1).getX(),
                    mPointsList.get(i + 1).getY(), mPointsList.get(i + 2)
                            .getX(), mPointsList.get(i + 2).getY());
        }
