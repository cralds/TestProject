...
public class CardSlidePanel extends ViewGroup {
    ...
    private List<Integer> dataList; // 存储的数据链表
...
//                viewItem.imageView.setOnClickListener(btnListener);
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onTouchClick() {
        	super.onTouchClick();
        	cardSwitchListener.onItemClick(null, isShowing);
        }
    }
...
//                CardDataItem dataItem = dataList.get(newIndex);
                changedView.fillData(dataList.get(newIndex));
            } else {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(action == MotionEvent.ACTION_UP){
        	if(ev.getX()-downX==0 && ev.getY()-downY==0){
        		cardSwitchListener.onItemClick(null, isShowing);
        	}
        }
     ...
     * @param dataList 数据
     */
    public void fillData(List<Integer> dataList) {
     ...
     */
    public void appendData(List<Integer> appendList) {
